...
public final class ApiKeyGenerator {
    ...
    public static ApiKey generate(String publicId) {
        ...
        final String key = generateSecret(ApiKey.API_KEY_LENGTH);
        final String fullKey = String.join(String.valueOf(ApiKey.API_KEY_SEPARATOR), prefix, publicId, key);

        final var apiKey = new ApiKey();
        apiKey.setClearTextKey(fullKey);
        apiKey.setPublicId(publicId);
        apiKey.setKey(hash(key));
        return apiKey;
    }
    ...
    /**
     * @param plainTextKey The plain text key to hash.
     * @return The hashed, hex-encoded key.
     * @since 3.2.0
     */
    public static String hash(final String plainTextKey) {
        try {
            final MessageDigest digest = MessageDigest.getInstance("SHA3-256");
            final byte[] hashedKey = digest.digest(plainTextKey.getBytes(StandardCharsets.UTF_8));
            return HexFormat.of().formatHex(hashedKey);
