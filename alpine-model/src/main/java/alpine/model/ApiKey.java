...
public class ApiKey implements Serializable, Principal {
     ...
     * @since 3.2.0
     */
    // TODO: Refactor last remaining usages in ApiKeyAuthenticationServiceTest, then remove.
    public static String getOnlyKey(String key) {
        if (key.length() == LEGACY_FULL_KEY_LENGTH) {
            return key;
        } else if (key.length() == LEGACY_WITH_PREFIX_FULL_KEY_LENGTH) {
            var startKey = PREFIX.length() + PUBLIC_ID_LENGTH;
            return key.substring(startKey);
        }

        String[] parts = key.split(Character.toString(API_KEY_SEPARATOR));
        if (parts.length != 3) {
